...
class Tool_Uploadgy{
	 ...
	 * $delete:  是否删除临时文件
	 */
	function Tool_Uploadgy($fileArray, $savePath = '', $delete = true){
	...
	function uploadh5($pImg,$tSavename,$tImgurl_small,$pCaseNo){
        ...
        $tDir = $this->savePath;
        $this->createDir(APPLICATION_PATH.'/public/'.$pCaseNo.'/'.$tImgurl_small);
		//Tool_Fnc::writefile(APPLICATION_PATH.'/public/'.$pCaseNo.'/'.$tImgurl_small.'/'.$tSavename.'.'.$this->extension,base64_decode($tData));
		$this->createDir($tDir);
		//Tool_Fnc::writefile($tDir.'/'/$this->saveName.'/'.$this->extension,$img_small);
		$res = Tool_Fnc::writefile($tDir.'/'.$this->saveName.'.'.$this->extension,base64_decode($tData));
		//$res = $this->createThumbh5('100','100',APPLICATION_PATH.'/public/'.$tImgurl_small);
        //return 1;
		if($res){
			return 1;
		}else{
...
*/
function reimageSize($imgsrc,$imgdst){ 
