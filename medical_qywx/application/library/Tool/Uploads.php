<?php
/**
 * Functionality: 多文件上传,生成缩略图,加水印类
 */
...
class Tool_Uploads{
	 ...
	 * $delete:  是否删除临时文件
	 */
	function Tool_Uploads($fileArray, $savePath = '', $delete = true){
		
        for($i = 0;$i<count($fileArray['name']);$i++){
		// Get original file info
		$this->fileName  = $fileArray['name'][$i];
		$this->tempName  = $fileArray['tmp_name'][$i];
		$this->getExtension($this->fileName);
		$this->size = $fileArray['size'][$i];
